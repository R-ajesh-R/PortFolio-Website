...
io.on("connection", (socket) => {
    socket.on('room',({roomNumber},cb)=>{
        console.log('check',roomNumber && Rooms.hasOwnProperty(roomNumber) && Object.keys(Rooms[roomNumber]).length >= 2,Rooms)
        if(roomNumber && Rooms.hasOwnProperty(roomNumber) && Object.keys(Rooms[roomNumber]).length >= 2){
        ...
        if(roomNumber){
            if(!Rooms.hasOwnProperty(roomNumber)){
                ...
                socket.join(roomNumber)
                socket.to(roomNumber).emit('room',{players:obj,turn:'X',roomNumber});
                socket.emit('room',{players:obj,roomNumber});
            }
        }
    })
    socket.on('made-move',({board,tracker,roomNumberValue,prevTurn,nextTurn})=>{
        socket.to(roomNumberValue).emit('made-move',{board,tracker,prevTurn,nextTurn})
    });
...
app.post('/api/email',authenticateToken,async(req,res)=>{
    try {
        ...
        res.status(200).json({ResponseStatus:"Message Sent Successfully!!",...sendEmail})
    } catch (error) {
